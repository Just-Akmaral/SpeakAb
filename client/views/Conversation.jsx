      ...
      return (
        <div>
          <ul className="clearfix">
            {this.props.words.map(word => <Word key={word} word={word} />)}
          </ul>
        </div>
      );
    }
      ...
      return (
        <div>
          <div className="conversation__task">
            ...
            <p className="text text-important">
              {this.getHint(this.props.curNumPhrase)}
            </p>
          </div>
          <div className="conversation__words">
            ...
            <Words
              words={this.getWords(
                this.props.curNumPhrase,
                this.props.numAttempt
              )}
            />
          </div>
        </div>
  ...
  render() {
    return (
      <div className="player">
        ...
        <button
          ref="runShowText"
          className="runShowText"
          style={{ display: "block" }}
          onClick={this.runShowText.bind(this)}
          className="player__showtext"
        />
        <span
          className={
            this.state.clicked ? "player__sound hidden" : "player__sound"
          }
        />
        <span
          ref="bot_speech"
          className={
            this.state.clicked ? "player__text" : "player__text hidden"
          }
        />
      </div>
    );
  ...
  run(event) {
    ...
    if (this.state.isEnd !== true) {
      ...
      if (this.state.isWrong) {
        // проверяем если этот ответ не правильный
        if (this.state.curNumAttempt === 2) {
          // если попытки уже было 3
      ...
      //может проверить на последнюю фразу?
    ...
    return (
      <section className="conversation clearfix">
        <section className="conversation__bot">
          ...
          <Audio
            scen={this.props.scenario}
            curNumPhrase={this.state.curNumPhrase}
          />
        </section>
        <section className="conversation__hints">
          <Hints
            numAttempt={this.state.curNumAttempt}
            scen={this.props.scenario}
            curNumPhrase={this.state.curNumPhrase}
        ...
        <section className="reply">
          <button
            ...
            className="btn btn-primary btn-mic"
          >
            Play
          </button>
          <div className="reply__record">
            <span className="text">record your reply</span>
            <p ref="user_speech" className="text" />
            <p ref="result" />
          </div>
          <div className={this.setClassResult()}>{this.showResult()}</div>
        </section>
        <section className={this.isHelp() ? "help" : "help hidden"}>
          <div className="help__popup">
            ...
            <button
              type="button"
              name="button"
              onClick={this.removeHelp.bind(this)}
              className="btn-close"
            >
              close
            </button>
...
class Conversation extends Component {
  render() {
    if (!this.props.ret || !this.props.map_obj) {
      return null;
    } else {
      return (
...
export default createContainer(props => {
  let conv = dbQuestsScenario.findOne({ name: props.location_id });
